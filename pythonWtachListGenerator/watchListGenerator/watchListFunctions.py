...
class WatchListFunctions:
    def mainWatchlistGeneratorFunction(self, showsToAdd, YOF):
        root = Tk()
        ...
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Watchlists")
        ...
        for show in showsToAdd:
            ...
            with open("pythonWtachListGenerator/watchListGenerator/Local DB/" + show, newline='') as csvfile:
                ...
                for cell in showCSV:
                    ...
                    sheet1.write(row, 8, str(_EC.EquationCreator.fixedPlaceEquation(row)), String_format)
                    sheet1.write(row, 9, str(_EC.EquationCreator.fixedSeasonAndEpisodeNumberEquation(row, 'B')), String_format)
                    sheet1.write(row, 10, str(_EC.EquationCreator.fixedSeasonAndEpisodeNumberEquation(row, 'C')), String_format)
                    sheet1.write(row, 11, str(_EC.EquationCreator.nameStickerEquation(row)), String_format_No_Align)
                    row = row + 1
    ...
    def generatAllWatchlists(self):
        ...
        for x in range(year, ThisYear):
            ...
            for filename in os.listdir(str(directory) + r"/pythonWtachListGenerator/watchListGenerator/Local DB"):
                if ".csv" in filename:
                    if _VF.VerifyingFunctions.checkIfContainsYear(filename, str(x)):
    ...
    def getBadDates(self):
        directory = pathlib.Path().absolute()
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Files")
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Local DB")
    ...
    def addShowClickedMed(self, Link):
    ...
    def addShowClicked(self, IMDBID):
        # initial variables
        directory = pathlib.Path().absolute()
        ia = IMDb()
        ...
        _PTMC.PythonToMySqlConnection.insertSingleEpisodeRecord(IMDBID)
        root.destroy()
        return cleanTitle + " Added successfuly"


    def getDateOfFirstEpisodeInList(self):
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Files")
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Local DB")
        today = date.today()
    ...
    def refreshShowStatus(self):
        ...
        directory = pathlib.Path().absolute()
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/Files")
        _VF.VerifyingFunctions.checkIfFolderExistAndCreate("pythonWtachListGenerator/watchListGenerator/TmpFiles")
